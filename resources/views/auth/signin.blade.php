...
<main class="grid flex-1 place-items-center bg-white px-6 py-24 sm:py-32 lg:px-8">
    <div class="w-full max-w-md text-center">
        ...
        @else
            <p class="mt-6 text-lg font-medium text-pretty sm:text-xl/8 text-gray-500">
                Continue to your account
            </p>
        @endif
        @foreach($providers as $provider)
            <div class="mt-10">
                <form id="signinForm" action="{{ $provider['signinUrl'] }}" method="POST" class="space-y-4">
                    @if($callbackUrl)
                        <input type="hidden" name="callbackUrl" value="{{ $callbackUrl }}">
                    @endif
                    <button id="signinButton" type="submit" class="flex w-full items-center justify-center gap-3 rounded-lg bg-blue-600 px-4 py-3 font-semibold text-white transition duration-200 hover:bg-blue-700 disabled:opacity-50 disabled:cursor-not-allowed">
